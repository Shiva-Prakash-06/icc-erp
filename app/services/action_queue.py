...
def build_action_queue(user, projects=None):
    ...
    dict shape must stay exactly as e2e's "oversight includes every pending
    category" test expects, since the queue's "Review" links resolve to
    these `tab`/`anchor` pairs. The ``tab`` values were re-pointed at the
    Tile System's five tabs on 2026-09-17; `erp._PROJECT_TAB_ALIASES` still
    resolves the old names, so a notification row written before the
    migration lands on the right tab rather than the default one.
    """
    if projects is None:
    ...
    for task in WorkTask.query.filter(WorkTask.project_id.in_(project_ids or [-1]), WorkTask.status == "Submitted").all():
        action_queue.append({"kind": "Task", "title": task.title, "project": task.project, "tab": "logistics", "anchor": task.public_id, "due_at": task.due_at,
    ...
    for item in checklist_items:
        action_queue.append({"kind": "Checklist", "title": item.template_item.title, "project": item.checklist.project, "tab": "logistics", "anchor": item.public_id, "due_at": item.due_at,
    ...
    for document in DocumentRecord.query.filter(DocumentRecord.project_id.in_(project_ids or [-1]), DocumentRecord.status == "Submitted").all():
        action_queue.append({"kind": "Document", "title": document.title, "project": document.project, "tab": "documents", "anchor": document.public_id, "due_at": None,
    ...
    for contribution in ContributionRecord.query.filter(ContributionRecord.project_id.in_(project_ids or [-1]), ContributionRecord.approval_status == "Pending").all():
        action_queue.append({"kind": "Contribution", "title": f"{contribution.person.display_name} · {contribution.activity_type}", "project": contribution.project, "tab": "people", "anchor": contribution.public_id, "due_at": None,
    ...
    for line in BudgetLine.query.filter(BudgetLine.project_id.in_(project_ids or [-1]), BudgetLine.status == "Submitted").all():
        action_queue.append({"kind": "Budget line", "title": f"{line.category} · {money(line.estimated_amount, currency=line.currency or 'INR')}", "project": line.project, "tab": "finance", "anchor": line.public_id, "due_at": None,
    ...
    for log in BuddyLog.query.join(BuddyAssignment).filter(BuddyAssignment.project_id.in_(project_ids or [-1]), BuddyLog.status == "Pending").all():
        action_queue.append({"kind": "Buddy log", "title": log.description[:100], "project": log.assignment.project, "tab": "people", "anchor": log.public_id, "due_at": None,
    ...
    for response in FeedbackResponse.query.join(FeedbackForm).filter(FeedbackForm.project_id.in_(project_ids or [-1]), FeedbackResponse.moderation_status == "Pending").all():
        action_queue.append({"kind": "Feedback moderation", "title": response.form.title, "project": response.form.project, "tab": "people", "anchor": response.public_id, "due_at": None,
    ...
    for snapshot in ReportSnapshot.query.filter(ReportSnapshot.project_id.in_(project_ids or [-1]), ReportSnapshot.approval_status == "Draft").all():
        ...
        action_queue.append({"kind": "Report approval", "title": snapshot.title, "project": snapshot_project, "tab": "documents", "anchor": snapshot.public_id, "due_at": None,
