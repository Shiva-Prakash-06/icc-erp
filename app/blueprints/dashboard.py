...
from app.models.erp import OperatingUnit, Person, RoleAssignment, Wing
...
def approve_user(user_id):
    ...
    try:
        ...
        assignment = _replace_scoped_assignment(user, assigned_role, request.form)
...
def modify_user_role(user_id):
    ...
    if new_role:
        try:
            ...
            user.role = new_role
            assignment = _replace_scoped_assignment(user, new_role, request.form)
            user.session_version += 1
...
def _replace_scoped_assignment(user, legacy_role, scope):
    ...
    if sensitive_requested and (not sensitive_allowed or not has_permission(g.user, "sensitive_links", sensitive=True)):
