...
Status: **`/erp` (the former hub) and `/erp/oversight` (the former Oversight page) both 302-redirect into the merged home at `/`** — see `docs/UI_UX_OVERHAUL_IMPLEMENTATION_PLAN.md` and `in-the-operation-checklists-crystalline-dongarra.md` Step 2. `/erp/oversight` retains its `approve`-permission 403 gate for non-approvers; `/erp` requires no permission (same as before). Home absorbs the portfolio KPI tiles, the 10-source decision queue (top 8 by default, `?queue=all` shows the full queue as a mode switch), and the person-scoped project/session/task/request lists that used to live on three separate pages.

| Route/template | Actors | Content/actions | Required states |
|---|---|---|---|
| `GET /erp` (→ 302 to `dashboard.index`) | Signed-in | — | — |
| `GET /erp/oversight` (→ 302 to `dashboard.index?queue=all`) | `approve` permission required | — | Non-approver: 403 |
...
| `GET /erp/projects/<public_id>?tab=overview|people|delivery|contributions|finance|insights|resources` · `erp/project_detail.html` | `can_view_project` | Project metadata, sessions, tasks, checklists, team, documents, lifecycle, report preview, split across 7 tabs (the former single `operations` tab is now `delivery`/`contributions`/`finance`; `?tab=operations` remains a permanent alias resolving to `delivery`) | Missing/403; zero sections; closure blockers; long task/checklist text; restricted reference redaction; stale versions; rejected/waived reason requirements; tables with row forms become records mobile; each operational section shows only items needing action by default, with settled rows and create forms behind a named disclosure |
